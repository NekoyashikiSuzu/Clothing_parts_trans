...
class FluxFillLoRATrainer:
    ...
    def _ensure_dtype_consistency(self, model, model_name=""):
        """确保模型数据类型一致性 - 保护embedding层和特殊缓冲区"""
        ...
        for name, param in model.named_parameters():
            # 跳过需要保持原始类型的层
            if any(skip in name.lower() for skip in ['embed', 'position_ids', 'token_type_ids']):
                logger.debug(f"跳过参数 {name} (保持类型: {param.dtype})")
                continue
                
            if param.dtype != self.target_dtype and param.dtype.is_floating_point:
        ...
        for name, buffer in model.named_buffers():
            if hasattr(buffer, 'dtype'):
                # 跳过索引类型的缓冲区
                if buffer.dtype in [torch.long, torch.int, torch.int32, torch.int64]:
                    logger.debug(f"跳过缓冲区 {name} (索引类型: {buffer.dtype})")
                    continue
                    
                if buffer.dtype != self.target_dtype and buffer.dtype.is_floating_point:
    ...
    def setup_models(self):
        ...
        with self.error_handler("LoRA设置"):
            ...
            self.validate_model_input_requirements()

    def validate_model_input_requirements(self):
        """验证并记录模型的输入要求"""
        logger.info("验证FLUX-Fill模型输入要求...")
        
        # 创建测试输入
        test_image = torch.randn(1, 3, 512, 512, device=self.accelerator.device, dtype=self.target_dtype)
        
        with torch.no_grad():
            # 测试VAE编码
            test_latents = self.vae.encode(test_image).latent_dist.sample()
            logger.info(f"VAE输出维度: {test_latents.shape}")  # 期望: [1, 16, 64, 64]
            
            # 测试transformer输入
            if hasattr(self.transformer, 'config'):
                config = self.transformer.config
                if hasattr(config, 'in_channels'):
                    logger.info(f"Transformer期望输入通道数: {config.in_channels}")
    ...
    def encode_prompt(self, prompt: str):
        ...
        with torch.no_grad():
            # 确保input_ids是正确的整数类型
            input_ids = text_inputs.input_ids.to(self.accelerator.device, dtype=torch.long)
            text_embeddings = self.text_encoder(input_ids=input_ids)[0].to(self.target_dtype)
        ...
        with torch.no_grad():
            # 确保input_ids是正确的整数类型
            input_ids_2 = text_inputs_2.input_ids.to(self.accelerator.device, dtype=torch.long)
            text_embeddings_2 = self.text_encoder_2(input_ids=input_ids_2)[0].to(self.target_dtype)
    ...
    def training_step(self, batch):
        """单步训练 - 修复FLUX-Fill输入格式和guidance参数"""
        with self.error_handler("训练步骤"):
            ...
            logger.info(f"VAE latents shape: {latents.shape}")  # [B, 16, 64, 64]
            ...
            # Flow Matching使用连续时间步 [0, 1]
            timesteps = torch.rand(batch_size, device=latents.device, dtype=torch.float32)
            timesteps_long = (timesteps * self.pipe.scheduler.config.num_train_timesteps).long()
            
            # Flow Matching的噪声混合
            t = timesteps.view(batch_size, 1, 1, 1).to(self.target_dtype)
            noisy_latents = (1 - t) * latents + t * noise
            
            # 创建mask区域的图像
            mask_binary = (mask_latents > 0.5).float()  # 二值化mask
            
            # 遮罩图像潜在表示：在mask区域使用噪声，其他区域使用原图
            masked_image_latents = noisy_latents * (1 - mask_binary) + noise * mask_binary
            
            # 创建64通道的遮罩
            # FLUX-Fill期望64通道的遮罩，我们需要重复单通道遮罩
            mask_64ch = mask_binary.repeat(1, 64, 1, 1).to(self.target_dtype)
            
            # 拼接所有输入：16 + 16 + 64 = 96通道
            # [图像潜在表示, 遮罩图像潜在表示, 64通道遮罩]
            model_input = torch.cat([noisy_latents, masked_image_latents, mask_64ch], dim=1)
            
            logger.info(f"Model input shape before packing: {model_input.shape}")  # [B, 96, 64, 64]
            
            # FLUX使用packing技术，需要确保高度和宽度是2的倍数
            # 然后进行patch embedding
            B, C, H, W = model_input.shape
            
            # FLUX的x_embedder期望384维输入
            # 根据错误信息，我们需要将96通道转换为384维特征
            # 这是通过2x2 patch embedding实现的：96 * 2 * 2 = 384
            patch_size = 2
            
            # 确保H和W能被patch_size整除
            assert H % patch_size == 0 and W % patch_size == 0, f"Height {H} and Width {W} must be divisible by patch_size {patch_size}"
            
            # 执行patch embedding
            # 从 [B, 96, 64, 64] -> [B, num_patches, 384]
            num_patches_h = H // patch_size  # 64 // 2 = 32
            num_patches_w = W // patch_size  # 64 // 2 = 32
            num_patches = num_patches_h * num_patches_w  # 32 * 32 = 1024
            
            # 重新排列为patches
            model_input_patches = model_input.unfold(2, patch_size, patch_size).unfold(3, patch_size, patch_size)
            # shape: [B, 96, 32, 32, 2, 2]
            
            model_input_patches = model_input_patches.permute(0, 2, 3, 1, 4, 5).contiguous()
            # shape: [B, 32, 32, 96, 2, 2]
            
            model_input_patches = model_input_patches.view(B, num_patches, C * patch_size * patch_size)
            # shape: [B, 1024, 384]
            
            logger.info(f"Model input patches shape: {model_input_patches.shape}")  # [B, 1024, 384]
            ...
            # 创建guidance参数 - FLUX-Fill需要这个参数
            # 训练时使用固定的guidance值
            guidance_scale = self.config.get("guidance_scale", 3.5)
            guidance = torch.tensor([guidance_scale], device=self.accelerator.device, dtype=self.target_dtype)
            guidance = guidance.expand(batch_size)
            
            # 前向传播
            try:
                # 检查transformer是否需要guidance
                if hasattr(self.transformer.config, 'guidance_embeds') and self.transformer.config.guidance_embeds:
                    # FLUX-Fill使用guidance
                    model_pred = self.transformer(
                        hidden_states=model_input_patches,  # [B, 1024, 384]
                        timestep=timesteps_long,
                        encoder_hidden_states=prompt_embeds,
                        pooled_projections=pooled_prompt_embeds,
                        guidance=guidance,  # 添加guidance参数
                        return_dict=False,
                    )[0]
                else:
                    # 如果不需要guidance（例如FLUX-schnell）
                    model_pred = self.transformer(
                        hidden_states=model_input_patches,  # [B, 1024, 384]
                        timestep=timesteps_long,
                        encoder_hidden_states=prompt_embeds,
                ...
                logger.error(f"  model_input_patches: {model_input_patches.shape}, {model_input_patches.dtype}")
                logger.error(f"  timesteps_long: {timesteps_long.shape}, {timesteps_long.dtype}")
                ...
                if hasattr(self.transformer.config, 'guidance_embeds') and self.transformer.config.guidance_embeds:
                    logger.error(f"  guidance: {guidance.shape}, {guidance.dtype}")
                raise
            
            # 将预测结果从patches格式转回原始格式
            # model_pred shape: [B, 1024, 384] -> [B, 96, 64, 64]
            if len(model_pred.shape) == 3 and model_pred.shape[1] == num_patches:
                # 反向patch embedding
                model_pred = model_pred.view(B, num_patches_h, num_patches_w, C, patch_size, patch_size)
                model_pred = model_pred.permute(0, 3, 1, 4, 2, 5).contiguous()
                model_pred = model_pred.view(B, C, H, W)
                
                # 只取前16个通道作为图像预测
                model_pred = model_pred[:, :16, :, :]
            
            # 计算损失 - Flow Matching的目标是速度场
            target = noise - latents
            target = target.to(self.target_dtype)
            
            # 只在mask区域计算损失
            mask_loss_weight = mask_binary.repeat(1, latents.shape[1], 1, 1)
            
            # 确保形状匹配
            if model_pred.shape != target.shape:
                logger.error(f"Shape mismatch: pred={model_pred.shape}, target={target.shape}")
                raise ValueError(f"Shape mismatch between prediction and target")
            ...
            loss = loss * mask_loss_weight.to(torch.float32)
            loss = loss.mean()
            loss = loss.to(self.target_dtype)
