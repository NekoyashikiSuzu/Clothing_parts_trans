...
class FluxFillLoRATrainer:
    ...
    def _ensure_dtype_consistency(self, model, model_name=""):
        """确保模型数据类型一致性 - 保护embedding层的indices"""
        logger.info(f"统一 {model_name} 数据类型到 {self.target_dtype}...")
        
        converted_params = 0
        converted_buffers = 0
        
        # 转换所有参数，但跳过embedding层的权重（保持原始类型）
        for name, param in model.named_parameters():
            # 跳过embedding层，这些需要保持原始精度
            if any(skip_name in name.lower() for skip_name in ['embed', 'position_ids']):
                continue
        ...
        # 转换所有缓冲区，但保护特殊的index缓冲区
        for name, buffer in model.named_buffers():
            if hasattr(buffer, 'dtype'):
                # 跳过position_ids等索引缓冲区
                if any(skip_name in name.lower() for skip_name in ['position_ids', 'token_type_ids']):
                    continue
                    
                if buffer.dtype != self.target_dtype:
    ...
    def training_step(self, batch):
        ...
        with self.error_handler("训练步骤"):
            ...
            timesteps = torch.rand(batch_size, device=latents.device, dtype=self.target_dtype)
            # 转换为scheduler期望的格式
            timesteps_long = (timesteps * self.pipe.scheduler.config.num_train_timesteps).long()
            
            # Flow Matching的噪声混合：x_t = (1-t) * x_0 + t * noise
            ...
            try:
                model_pred = self.transformer(
                    ...
                    timestep=timesteps_long,  # 使用long类型的timesteps
            ...
            # 计算损失 - Flow Matching的目标是速度场 (velocity)
            # Flow Matching的目标：v = noise - latents
