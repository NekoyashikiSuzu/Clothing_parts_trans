...
class FluxFillLoRATrainer:
    ...
    def training_step(self, batch):
        ...
        with self.error_handler("训练步骤"):
            ...
            # 记录latents形状
            logger.debug(f"VAE latents shape: {latents.shape}")  # 应该是 [B, 16, 64, 64]
            ...
            # FLUX-Fill的输入准备 - 不要拼接，而是使用mask来修改latents
            # 在mask区域填充随机噪声或特定值
            mask_binary = (mask_latents > 0.5).float()  # 二值化mask
            
            # 创建masked输入：在mask区域使用纯噪声
            masked_noise = torch.randn_like(noisy_latents)
            model_input = noisy_latents * (1 - mask_binary) + masked_noise * mask_binary
            
            # 确保输入维度正确
            logger.debug(f"Model input shape: {model_input.shape}")  # 应该是 [B, 16, 64, 64]
            ...
            try:
                model_pred = self.transformer(
                    hidden_states=model_input,  # 现在是正确的16通道
                ...
                if hasattr(self.transformer, 'x_embedder'):
                    logger.error(f"  x_embedder期望的输入: {self.transformer.x_embedder.weight.shape}")
                raise
